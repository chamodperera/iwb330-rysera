...
export default function Dashboard() {
  ...
  const [estimatedValues, setEstimatedValues] = useState<{
    [key: number]: { price: number; time: string; weight: number } | null;
  }>({});
      ...
      <Card className="p-6 w-1/5 hidden lg:block">
        <div className="mb-2">
          <img
            ...
            alt={
              selectedMaterial
                ? `${selectedMaterial.name} Filament`
                : "Material"
            }
          ...
          {selectedMaterial && (
            <div>
              ...
              <p className="text-sm">
                ...
                {selectedMaterial.properties.Tensile}
              </p>
              <p className="text-sm">
                <strong>Tensile elongation:</strong>{" "}
                {selectedMaterial.properties.elongation}
              </p>
              <p className="text-sm">
                <strong>Flexural strength:</strong>{" "}
                {selectedMaterial.properties.Flexural}
              </p>
              <p className="text-sm">
                <strong>HDT (0.45 MPa):</strong>{" "}
                {selectedMaterial.properties.HDT}
              </p>
            </div>
          )}
        </CardContent>
            ...
            <>
              ...
              {fileStates[currentFileIndex].status === "uploading" ? (
                <div className="w-full h-full flex flex-col items-center justify-center">
                  <p className="mt-4">
                    Uploading:{" "}
                    {fileStates[currentFileIndex].uploadProgress.toFixed(0)}%
                  </p>
                </div>
